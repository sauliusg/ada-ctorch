...
with Torch.Rng_Init; -- initialise the Random Number Generator for Torch
...
procedure Mnist_Stddecl is
   
   -- This is an experiment with the MNIST network using the
   -- "standard" Ada declaration order. A special trick is used to
   -- seed the Torch Random Number Generator (RNG) early.
   
   Rng_Initializer : Torch.Rng_Init.Rng_Initializer (1);
   ...
   Train_Mnist_Dataset : Torch.Datasets.Dataset :=
     Make_Stacked
       (
        Make_Normalised_MNIST (Root_Dir, 0.1307, 0.3081, Mode => Train)
       );
